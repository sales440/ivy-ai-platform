...
## Phase 59: Critical - ROPA Company/Campaign/Draft Integration Broken (COMPLETED)
- [x] Fix ROPA cannot create companies in DB (DB connection pool with auto-reconnect)
- [x] Fix ROPA doesn't see manually created companies (unified localStorage → DB)
- [x] Fix campaigns generated by ROPA not linked to manually created companies (added clientId/companyName to salesCampaigns)
- [x] Fix email drafts not generated when ROPA is asked to create them for PETLIFE 360 (Brand Firewall uses dynamic profiles)
- [x] Fix ROPA should read PETLIFE 360 logo from Google Drive for email templates (CDN logo configured)
- [x] Verify company creation writes to actual database (tRPC clients.create endpoint)
...
- [x] Commit completo a GitHub con token ghp_ (sales440/ivy-ai-platform)
- [x] Push y deploy en Railway upbeat-creativity-production (deployment activo)
